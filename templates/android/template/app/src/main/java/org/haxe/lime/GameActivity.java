...
// this is needed so that the FMod stuff gets initialized before the application (likely just libApplicationMain.so)
...
public class GameActivity extends SDLActivity {
	...
	protected void onCreate (Bundle state) {
		...
		List<Boolean> calledExtensions = new ArrayList<>();
		...
		try {
			...
			for (int i = 0; i < extensions.size(); i++) {
				...
				extension.onCreate (state);
				calledExtensions.set(i, true);
				Log.d("GameActivity", "Initialized an extension earlier!");
			}
		}
		catch (Exception e) {
			Log.d("GameActivity", "Tried to initialized an extension earlier but failed.");
		}
